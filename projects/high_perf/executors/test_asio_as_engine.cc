...
void handleAccept(const boost::system::error_code &error, boost::shared_ptr<boost::asio::ip::tcp::socket> socket,
...
void startAccept(boost::asio::ip::tcp::acceptor &acceptor) {
    ...
    acceptor.async_accept(*socket,
                          boost::bind(handleAccept, boost::asio::placeholders::error, socket, boost::ref(acceptor)));
}
...
public:
    // bind
    async_udp_server(boost::asio::io_service &io_service) : m_socket(io_service), m_buffer(new vector<char>(1024, 0)) {
    ...
    // m_socket.shutdown();
    ~async_udp_server() { m_socket.close(); }

private:
    void start_receive() {
        m_socket.async_receive_from(buffer(&(*m_buffer)[0], m_buffer->size()), sender_endpoint,
                                    bind(&async_udp_server::handle_receive_from, this, boost::asio::placeholders::error,
    ...
    void handle_receive_from(const boost::system::error_code &error, std::size_t bytes_transferred) {
        if (!error && bytes_transferred > 0) {
            ...
            m_socket.async_send_to(
                buffer(*message), sender_endpoint,
                bind(&async_udp_server::handle_send_to, this, message, boost::asio::placeholders::error,
                     boost::asio::placeholders::bytes_transferred));
        } else {
            start_receive();
        }
    ...
    /*error*/
    /*bytes_transferred*/
    void handle_send_to(boost::shared_ptr<std::string>, const boost::system::error_code &, std::size_t) {
        start_receive();
    }
...
public:
    // bind
    async_udp_client(boost::asio::io_service &io_service) : m_socket(io_service), m_buffer(new vector<char>(1024, 0)) {
        m_socket.open(udp::v4());
    ...
    // http://stackoverflow.com/questions/15027268/how-to-close-boost-asio-server-socket-with-all-client-sockets-connected
    // m_socket.shutdown();
    ~async_udp_client() { m_socket.close(); }
    ...
    void send() {
        ...
        // async_send not compiled
        udp::endpoint target;
        ip::address targetIP = ip::address::from_string("127.0.0.1", myError);
        target.address(targetIP);
        target.port(1036);
        m_socket.async_send_to(buffer(*message), target,
                               bind(&async_udp_client::handle_send_to, this, message, boost::asio::placeholders::error,
                                    boost::asio::placeholders::bytes_transferred));
    }
...
private:
    void start_receive() {
        m_socket.async_receive_from(buffer(&(*m_buffer)[0], m_buffer->size()), sender_endpoint,
                                    bind(&async_udp_client::handle_receive_from, this, boost::asio::placeholders::error,
                                         boost::asio::placeholders::bytes_transferred));
    }
