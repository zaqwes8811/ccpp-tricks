/**
  ...
  From Chromium:
    ...
    These models are "threads + mutexes + condition-variables", and "threads +
  message-passing"
...
TEST(AsPl, SingleThreadShared) {
...

    {
        ...
        SingleWorker::Callable c0 = boost::bind(&boost::packaged_task<int>::operator(), boost::ref(t0));

        SingleWorker::Callable c1 = boost::bind(&boost::packaged_task<int>::operator(), boost::ref(t1));
