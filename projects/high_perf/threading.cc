...
std::string current_thread_id() { return boost::lexical_cast<std::string>(this_thread::get_id()); }

void executors::terminate_all() {
    using namespace std;
    using namespace boost;
    lock_guard<recursive_mutex> _(mtx_);
    for (map<int, weak_ptr<executor>>::iterator it = id_to_exec_.begin(); it != id_to_exec_.end(); ++it) {
...
//==========================================================
asio_executor::asio_executor() { thread_.reset(new boost::thread(boost::bind(asio_executor::real_run, &io_))); }
...
void asio_executor::terminate() { io_.stop(); }
