...
public:
    work_executor(boost::asio::io_service *io) { this->io = io; }
...
class executors {
public:
    // fixme: Очень проблеманя функция - она блокирует поток, не ясно как в
    // некоторых других
    //   случаях получить ответ из другого потока
    template <typename T, typename Func>
    static T post(int id, Func func) {
        using namespace boost;
        shared_ptr<packaged_task<T>> t = shared_ptr<packaged_task<T>>(new packaged_task<T>(func));
