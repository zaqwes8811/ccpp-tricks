...
template <typename T>
class UnifiedMessageView {
public:
    UnifiedMessageView(SharedMessage msg) : weak_msg_{msg} { data_ptr_ = std::any_cast<T>(msg.get()); }
...
std::unique_ptr<IOrderManager> Manager() { return std::make_unique<OrderManager<OrderSenderA>>(); }
