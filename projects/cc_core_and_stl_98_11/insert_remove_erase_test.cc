...
// copy(... inserter/back_inserter/front_ins) -> должны заменяться интервальными
// функциями, insert, например
...
bool bad_value(int x) { return x < 4; }

bool rm_bad_value(int x) { return x == 7; }
...
TEST(STL, RemoveInsertErase) {
  ...
  v1.erase(std::remove(v1.begin(), v1.end(), 1),
           boost::end(v1)); // что-то не то
  cout << v1;

  //
  v1.erase(std::remove_if(v1.begin(), v1.end(), bad_value),
           boost::end(v1) // !! Very important!! Without in compiled and work
                          // but it's incorrect
  );
  cout << v1;
...
TEST(STL, Remove_fun) {
  ...
  vector<int>::iterator next =
      coll.erase(pos, coll.end()); // Возвращает VALIDE(!!!) итератор
  ...
  cout << coll;
  vector<int> e;
  remove_copy_if(coll.begin(), coll.end(), back_inserter(e),
                 bind2nd(less<int>(), 4));
  cout << e << coll;
  assert(coll == c_save);
...
// важно что происходит и удаление и обход. может быть цикл слишком много на
// себя берет?
