...
TEST(Viva64Test, L5) {
    ...
    size_t size = v.size();
    for (ptrdiff_t  // signed! delta can be > 0 and < 0
                    // size_t
             k = 0;
         k < size; ++k) {
        // v[k];
...
bool FooFind(char *Array, char Value, size_t Size) {
    ...
    for (unsigned i = 0; i != Size; ++i)
        if (i % 5 == 0 && Array[i] == Value) return true;
    return false;
}
...
TEST(Viva64Test, L9) {
    ...
    memset(values, 0, ARRAY_SIZE * sizeof(size_t)  // better sizeof(values)
                                                   // 4 // BAD
    );
...
TEST(Viva64Test, L13) {
    {
        ...
        pointer += static_cast<ptrdiff_t>(a16) * static_cast<ptrdiff_t>(b16) * static_cast<ptrdiff_t>(c16);
...
float Region::GetCell(TCoord x, TCoord y, TCoord z) const { return array[x + y * Width + z * Width * Height]; }
...
TEST(Viva64Test, L17) {
    ...
    {
        ptrdiff_t val_1 = -1;
        unsigned int val_2 = 1;  // extend to greater! it's diff on platforms
        if (val_1 > val_2)
...
TEST(Viva64Test, L23) { cout << sizeof(MyStructSmall) << endl; }
