...
// Intel TBB
template <typename T> class concurent_queue_ : public boost::noncopyable {
public:
  concurent_queue_() {}
...
*/
template <typename T> class concurent_try_queue : public boost::noncopyable {
public:
  // types
  ...
  explicit concurent_try_queue(int size)
      : cm_size(size), m_nblocked_pop(0), m_nblocked_push(0),
        m_with_blocked(false), m_stopped(false), m_current_size(0) {
    if (cm_size < 1) {
