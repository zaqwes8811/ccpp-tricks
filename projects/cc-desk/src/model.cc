...
void lock(Model::TaskCell &r_cell) { r_cell.first = true; }

bool try_lock(Model::TaskCell &r_cell) { return r_cell.first == false; }

void unlock(Model::TaskCell &r_cell) { r_cell.first = false; }

namespace ext {
void onNew(gc::SharedPtr<Model> m, TaskEntity task_ptr, Task saved_task) {
  *task_ptr = saved_task;
  auto iter = std::find_if(begin(m->m_task_cells), end(m->m_task_cells),
                           [task_ptr](models::Model::TaskCell v) {
                             return task_ptr->id == v.second->id;
                           });

  if (iter == end(m->m_task_cells))
    return;
...
void Model::initialize(std::function<void(std::string)>) {
  ...
  Workers::PostTask(Workers::DB, [model, db]() mutable {
    auto tables = vector<table_concept_t>{db->getTaskTableQuery(),
                                          db->getTagTableQuery()};
    ...
    Workers::PostTask(Workers::UI,
                      bind(bind(&Model::OnLoaded, model, _1), tasks));
  });
}

Model::TaskCell Model::GetCachedTaskById(const size_t id) {
  auto it = std::find_if(m_task_cells.begin(), m_task_cells.end(),
...
void Model::AppendNewTask(const Task &unsaved_task) {
  ...
  auto on_success =
      std::bind(&ext::onNew, shared_from_this(), unsaved_task_ptr, _1);
...
entities::TaskEntities Model::FilterModelData() {
  ...
  std::transform(
      begin(m_task_cells), end(m_task_cells), std::back_inserter(r),
...
void Model::NotifyObservers() { m_observer_ptr->update(FilterModelData()); }
