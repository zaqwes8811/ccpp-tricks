...
bool Row::isPresent() const { return idx != -1 && idx >= 0; }
...
Row Row::absent() { return Row(); }

Row Row::of(int v) { return Row(v); }

void UiEngine::onUiLoaded() {
  m_modelPtr->initialize([](std::string) { ; });
}

void UiEngine::onClose() { m_fsmToDestroy = true; }
    ...
    : QMainWindow(parent) //, m_scope(s)
{
  ...
  // FIXME: сделать что-то через editor - пока тщетно. Например как
  // сконнектиться с отнасл. таблицей?
  ...
  connect(sort_dec, SIGNAL(stateChanged(int)), this,
          SLOT(onOnOffSortByDecPriority(int)));
  checkbox_layout->addWidget(sort_dec);

  auto done = new QCheckBox("Sort by task name", this);
  connect(done, SIGNAL(stateChanged(int)), this,
          SLOT(onOnOffSortByTaskName(int)));
  ...
  connect(m_taskTablePtr, SIGNAL(itemChanged(QTableWidgetItem *)), this,
          SLOT(onRowIsChanged(QTableWidgetItem *)));

  // make checkbox
  // connect(_table->horizontalHeader(), SIGNAL(sectionClicked(int)), this,
  // SLOT(filterOnOffSortByDecPriority(int)));
...
void UiEngine::processFilter(filters::FilterPtr f, int state) {
  if (Qt::Unchecked == state)
    m_modelPtr->removeFilter(f);
  if (Qt::Checked == state)
    m_modelPtr->addFilter(f);
}
...
void UiEngine::closeEvent(QCloseEvent *event) { onClose(); }
...
gc::SharedPtr<UiEngine> UiEngine::share() { return shared_from_this(); }

bool UiEngine::isReadyToDestroy() const { return m_fsmToDestroy; }
