...
std::function<bool(entities::TaskEntity)> is_non_saved() {
  return bind(bind(equal_to<size_t>(), _1, EntityStates::kInactiveKey),
              bind(&Task::id, _1));
}
...
void ChainFilters::add(FilterPtr e) { m_s.insert(e); }

// FIXME: как удалить то без RTTI? Список то полиморфный
void ChainFilters::remove(FilterPtr e) { m_s.erase(e); }

entities::TaskEntities
ChainFilters::operator()(entities::TaskEntities e) const {
  auto r = e;
  for (auto &action : m_s)
    r = (*action)(r);
  return r;
...
int DoneFilter::typeCode() const { return 1; }

entities::TaskEntities
SortByPriorityFilter::operator()(entities::TaskEntities e) {
  std::stable_sort(e.begin(), e.end(),
                   bind(std::greater<int>(), bind(&Task::priority, _1),
                        bind(&Task::priority, _2)));
  return e;
}

int SortByPriorityFilter::typeCode() const { return 2; }
...
entities::TaskEntities SortByTaskName::operator()(entities::TaskEntities e) {
  std::stable_sort(e.begin(), e.end(),
                   bind(std::greater<string>(), bind(&Task::name, _1),
                        bind(&Task::name, _2)));
  return e;
}

int SortByTaskName::typeCode() const { return 3; }
