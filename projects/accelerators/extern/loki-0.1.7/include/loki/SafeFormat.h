...
#define LOKI_PRINTF_STATE_FORWARD(type)                                        \
  ...
  PrintfState &operator()(LOKI_SAFEFORMAT_UNSIGNED_LONG i) {
    ...
    if (ForceShort()) {
      ...
      if (c == 'x' || c == 'X' || c == 'u' || c == 'o') {
        i = static_cast<LOKI_SAFEFORMAT_UNSIGNED_LONG>(
  ...
  PrintfState &operator()(const char *const s) {
    ...
    if (fmt == 'p') {
      FormatWithCurrentFlags(
          reinterpret_cast<LOKI_SAFEFORMAT_UNSIGNED_LONG>(s));
      return *this;
    }
    if (fmt != 's') {
      result_ = -1;
...
private:
  ...
  template <typename T> PrintfState &StoreCountHelper(T *const pi) {
    ...
    if (fmt == 'p') { // pointer
      FormatWithCurrentFlags(
          reinterpret_cast<LOKI_SAFEFORMAT_UNSIGNED_LONG>(pi));
      return *this;
    }
    if (fmt != 'n') {
      result_ = -1;
  ...
  void FormatWithCurrentFlags(const LOKI_SAFEFORMAT_UNSIGNED_LONG i) {
    ...
    Char buf[sizeof(LOKI_SAFEFORMAT_UNSIGNED_LONG) * 3 // digits
             + 1                                       // sign or ' '
             + 2                                       // 0x or 0X
             + 1];                                     // terminating zero
    ...
    if (formatChar == 'c') {
      ...
      *bufLast = static_cast<char>(i);
      ...
      const bool negative =
          isSigned && static_cast<LOKI_SAFEFORMAT_SIGNED_LONG>(i) < 0;
      if (formatChar == 'o')
        base = 8;
      ...
      bufLast =
          isSigned
              ? RenderWithoutSign(static_cast<LOKI_SAFEFORMAT_SIGNED_LONG>(i),
                                  bufLast, base, formatChar == 'X')
  ...
  char *RenderWithoutSign(LOKI_SAFEFORMAT_SIGNED_LONG n, char *bufLast,
                          unsigned int base, bool uppercase) {
    if (n != LONG_MIN) {
      return RenderWithoutSign(
          static_cast<LOKI_SAFEFORMAT_UNSIGNED_LONG>(n < 0 ? -n : n), bufLast,
          base, uppercase);
    }
