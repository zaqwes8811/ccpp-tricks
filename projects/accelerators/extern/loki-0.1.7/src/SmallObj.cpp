...
bool Chunk::IsBlockAvailable(void *p, unsigned char numBlocks,
  ...
  unsigned char blockIndex =
      static_cast<unsigned char>((place - pData_) / blockSize);
...
FixedAllocator::FixedAllocator()
    : blockSize_(0), numBlocks_(0), chunks_(0), allocChunk_(NULL),
      deallocChunk_(NULL), emptyChunk_(NULL) {}
...
SmallObjAllocator::SmallObjAllocator(std::size_t pageSize,
    ...
    : pool_(NULL), maxSmallObjectSize_(maxObjectSize),
