...
Thread::Thread(volatile ThreadPool *owner)
    : m_thread(), m_func(nullptr), m_parm(nullptr), m_status(Thread::Idle),
      m_stop(false), m_owner(owner) {
  assert(IsValid(owner));
}

// ----------------------------------------------------------------------------

Thread::Thread(volatile ThreadPool *owner, CallFunction function, void *parm)
    : m_thread(), m_func(function), m_parm(parm), m_status(Thread::Starting),
      m_stop(false), m_owner(owner) {
  assert(IsValid(owner));
...
ThreadPool::ThreadPool(unsigned int threadCount) : m_threads() {
  assert(IsValid());
...
volatile const Thread *ThreadPool::GetThread(unsigned int index) const
    volatile {
  assert(IsValid());
  LOKI_DEBUG_CODE(Checker checker(this); (void)checker;)
  ThreadPool *pThis = const_cast<ThreadPool *>(this);
