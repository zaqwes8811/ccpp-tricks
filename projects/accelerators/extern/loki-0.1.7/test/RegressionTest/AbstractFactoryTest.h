...
typedef Loki::ConcreteFactory<
    AbstractEnemyFactory, Loki::OpNewFactoryUnit,
    Seq<BadSoldier, BadMonster, BadSuperMonster>::Type>
    HardLevelEnemyFactory;
...
public:
  ...
  virtual void execute(TestResult &result) {
    ...
    r = !!(
        typeid(*s) ==
        typeid(
            SillySoldier)); // SGB !! eliminates bool-to-int performance warning
    ...
    s = hardFactory
            ->Create<Soldier>(); // BCB bug!!! - always creates SillySoldier
