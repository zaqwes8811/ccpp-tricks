...
//
// I = T (A::* [const] [volatile] [&])([P1, P2, ---, Pn] [...]]) [const]
// [volatile]
//
// Repetition
// ----------
...
#define TEST_CONDITION(cond, answer) testCondition(cond, answer, #cond);

#define TEST_CONDITION_HELP(type, cond, answer)                                \
  TEST_CONDITION(Loki::TypeTraits<type>::cond, answer)

#define PASS(type) TEST_CONDITION_HELP(type, CONDITION, true)

#define FAIL(type) TEST_CONDITION_HELP(type, CONDITION, false)
...
#define TEST_TYPE_HELP2(type, rightType, cond)                                 \
  {                                                                            \
    bool testResult =                                                          \
        Loki::IsSameType<rightType, Loki::TypeTraits<type>::cond>::value;      \
...
#define TEST_TYPE(type, rightType) TEST_TYPE_HELP(type, rightType, CONDITION)
...
public:
  TypeTraitsTest2()
      : Test("TypeTraits.h"), testedConditions_(0), erroneousConditions_(0),
        groupErrors_(0), textBuffer_(), groupName_() {}
...
private:
  ...
  void testType(bool result, std::string conditionText, std::string typeText,
...
inline void TypeTraitsTest2::execute(TestResult &result) {
  ...
  cout << endl
       << "Tested " << testedConditions_ << " conditions of which "
       << erroneousConditions_ << " were erroneous" << endl;
...
inline void TypeTraitsTest2::testEnd() {
  ...
  if (groupErrors_ > 0) {
    cout << endl;
    cout << groupName_ << ": " << groupErrors_ << " error(s) found." << endl;
    cout << "---------------------------"
         << "-----------------------" << endl;
...
inline void TypeTraitsTest2::testCondition(bool result, bool answer,
                                           std::string text) {
  ...
  message +=
      string("    Result: ") + string(result ? "true" : "false") + string("\n");
  message += string("    Expected: ") + string(answer ? "true" : "false") +
             string("\n");
...
#define CONDITION PointeeType
  ...
  TEST_TYPE(void *, void);
  TEST_TYPE(const volatile int *, const volatile int);
  TEST_TYPE(const volatile int *const volatile &, const volatile int);
  TEST_TYPE(void (*)(int, float), void(int, float));
...
#define CONDITION NonConstType
  ...
  TEST_TYPE(const volatile int *const volatile, const volatile int *volatile);
  TEST_TYPE(int(int, float, ...), int(int, float, ...));
  TEST_TYPE(int (*const volatile)(int, float, ...),
            int (*volatile)(int, float, ...));
  TEST_TYPE(int(A::*const volatile)(int, float, ...),
...
#define CONDITION NonVolatileType
  ...
  TEST_TYPE(const volatile int *const volatile, const volatile int *const);
  TEST_TYPE(int(int, float, ...), int(int, float, ...));
  TEST_TYPE(int (*const volatile)(int, float, ...),
            int (*const)(int, float, ...));
  TEST_TYPE(int(A::*const volatile)(int, float, ...),
