...
template <typename T> void Mallocator<T>::destroy(T *const p) const { p->~T(); }
...
TEST(Alloc, Lavavej) {
  ...
  for (vector<int, Mallocator<int>>::const_iterator i = l.begin(); i != l.end();
       ++i) {
    cout << "Element: " << *i << endl;
  }
...
public:
  ...
  virtual void playNote(Note key) { cout << "Piano " << key << endl; }
};
...
public:
  ...
  virtual void playNote(Note key) { cout << "Violin " << key << endl; }
};
