...
enum MessageType { INT32 = 0, DOUBLE };

template <MessageType> struct Map;

template <> struct Map<INT32> {
  using type = int32_t;
...
template <MessageType A> typename Map<A>::type map_attr() {
  return Map<A>::value;
}
...
template <enum MessageType T, typename V> auto special_get(const V &v) {
...
TEST(Cpp, VariantEnumAny) {
  ...
  Value v = 8;
  EXPECT_THROW(std::get<decltype(map_attr<DOUBLE>())>(v),
...
template <typename T> void set_value_gen(T v) {
...
void set_value(int32_t v) { set_value_gen(v); }

void set_value(double v) { set_value_gen(v); }
