...
template <class T, int size> T min(const T (&r_array)[size]) {
  T min_val = r_array[0];
...
template <class T> T foo(T *val); // int *T -> T* val

template <class T1, typename T2, class T3> T1 foo(T2, T3);

/*inline*/ template <typename T> inline T foo(T, unsigned int *);

template <class myT /*, class myT*/> void foo(myT, myT);
...
template <class Ctype> Ctype foo(Ctype a, Ctype b);
...
template <class T> void putValuesTempl(T *rarray, int sz) {
  using std::cout;
  cout << "( " << sz << " )< ";
  for (int i = 0; i < sz; ++i) {
    if (i % lineLength == 0 && i)
...
namespace n1 {
// failed deduction
template <typename T> void meow(T t, T two) { cout << t << " " << two << endl; }
...
namespace n3 {
template <typename T>
void meow(typename make_unsigned<T>:: // !! source  dest. int  unsigned int
          type t) {
...
void meow(T t,
          // function<void(T)>
          typename Ident<function<void(T)>>::type f) {
  f(t); // << u << endl;
}
...
template <typename T> void do_thing(T vPtr) { vPtr->foo(); }
