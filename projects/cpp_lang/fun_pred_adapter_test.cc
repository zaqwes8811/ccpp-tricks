...
//
// Note: В алгоритм можно передать и по ссылке, но тогда нужно полностью
// специфицировать шаблон.

// функторы передаются по значению
//   они должны быть малы и они не могут быть полиморфными. можно обернуть не
//   удовл. классы.
...
public:
    ...
    // не должны менятся свойства как предиката, но константность лучше сперва
    // поставить
    bool operator()(int) /* must be const, but accamulate? best make const */ { return ++count == nth; }
};
