...
TEST(STL, Strings) {
  ...
  string::size_type idx =
      filename.find('.'); // возвращает позицию первого совпадения
  assert(idx != string::npos);
  // assert(distance(idx, string::npos) == 4);  // не компилируется
  assert(typeid(idx) == typeid(string::size_type)); // !!

  string base_name =
      filename.substr(/*1000 - std::out_of_range*/ 0,
                      (idx + 1) - 1); // второй аргумент - количество символов!
  ...
  string::size_type beg_idx =
      for_tream.find_first_not_of(delims); // поиск с начала?
  ...
  while (beg_idx != string::npos) {
    ...
    words.push_back(
        for_tream.substr(beg_idx, end_idx - beg_idx)); // [begin, N штук

    // Но кажется лучше не испоьзовать npos and size_type
    // for (int i = end_idx-1; i >= static_cast<int>(beg_idx);  // приведение
    // типов обязательно!!
  ...
  //   - нужно учитывать, если используются ссылки, указатели или итераторы или
  //   если важна производительность
  // [] - работает по разному для const and non-const string
  //
  // TODO: WTF?
  // https://www.sgi.com/tech/stl/basic_string.html - "has very unusual iterator
  // invalidation semantics" begin, end для неконстант могут стать невалидной!!
  ...
  //  ICU in embedded world -
  //  http://thebugfreeblog.blogspot.ru/2013/05/cross-building-icu-for-applications-on.html
}
