...
typedef enum helper_Result { FOO_OK, FOO_ERROR1, FOO_ERROR2, FOO_UNKNOWN } helper_Result;
...
int helper_boundaryNoThrowGuarantee_c() {
    try {
        ...
        if (!fi.is_ready() || fi.has_exception()) return -1;
...
TEST(PureCBoundCC, Boundary) { helper_boundaryNoThrowGuarantee_c(); }
