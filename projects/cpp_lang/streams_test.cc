...
void restore_stream_flags(ostream &o, ios::fmtflags old_flags) { o.flags(old_flags); }
...
TEST(STL, StreamsTest) {
    ...
    //
    // TODO: обработка ошибок - классы разработаны до появл. искл. в станд., но
    // можно сделать так
    //   чтобы при установке флагов генерировались исключения. Лучше пусть не
    //   генерят, а генерить пользовательские.
    // Генерить исключения при установке любого флага.
    // stream.exceptions(std::ios::eofbit | ios::failbit | ios::bacbit);
    //
    // Есть флаги состояния. Похоже при ощибках устанавливаются они, а не
    // бросаются исключения.
    //  Есть обертки проверки условий operator!(), например, или operator void*()
    //  - но вообще вроде бы это не хорошо
    //
    ...
    // Buffers
    {
        ...
        // на буффер не влияет, так что можно передать буффер в функции, а потом
        // настраивать поток это влияние на буффер не окажет
...

    {
        ...
        ofstream out("testfile.txt",
                     ios::in | ios::out);  // но откр. для чтения И записи
