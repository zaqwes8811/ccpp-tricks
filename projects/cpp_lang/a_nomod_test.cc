...
TEST(STL, Find) {
    ...
    if (pos1 != coll.end()) pos2 = find(++pos1, coll.end(), 4);  // нач. со след. за найденным

    // print
    if (pos1 != coll.end() && pos2 != coll.end()) copy(--pos1, ++pos2, ostream_iterator<int>(cout, " "));
    cout << endl;
}
...
TEST(STL, SearchSubInterval) {
    ...
    rpos = find_first_of(coll.rbegin(), coll.rend(), subcoll.begin(), subcoll.end());
    assert(*rpos == 6);
}
...
TEST(STL, AllInOne) {
    ...
    // equal - mismatch
    {
        ...
        pair<vector<int>::iterator, vector<int>::iterator> finded = mismatch(first.begin(), first.end(), v.begin());
