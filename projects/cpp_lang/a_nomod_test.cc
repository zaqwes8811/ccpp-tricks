...
TEST(STL, SearchNBySomeCriteria) {
  ...
  pos = search_n(coll.begin(), coll.end(), 4, 3,
                 greater<int>()); // no STL way. Причем предикат бинарный
...
TEST(STL, SearchSubInterval) {
  ...
  rpos =
      find_first_of(coll.rbegin(), coll.rend(), subcoll.begin(), subcoll.end());
  assert(*rpos == 6);
}
...
// p. 351
bool doubled(int elem, int nextElem) { return elem * 2 == nextElem; }
...
struct pred_upper_bound {
  ...
  bool operator()(const int value) const { return value < boundary; }
};
...
TEST(STL, AllInOne) {
  ...
  assert(v.end() != find_if(v.begin(), v.end(), bind2nd(greater<int>(), 7)));

  // поиск подряд идущих
  std::vector<int>::iterator it = search_n(v.begin(), v.end(), 2, 7);
  assert(it == v.end()); // нет заданного числа повторений

  // вставка перед элеметом на который указывает - отодвигает элемент, на кот.
  // указывает
  v.insert(
      v.begin() + 7,
      7); // не действительными могут стать итер. как все после, так и вообще
  v.insert(v.end(), 17); // перед end!
  cout << v;

  assert(v.end() == search_n(v.begin(), v.end(), 4, 9,
  ...
  /// поиск паттерна
  {
    ...
    while (true) {
      it = search(start, v.end(), pattern.begin(), pattern.end());
      ...
      // processing
      cout << vector<int>(
          it, start); // интервал валдный, т.к. раз нашли, значит все норм
      ++count;
    }
    assert(count == 2);
  ...
  while (true) {
    it = find_end(v.begin(), stop, pattern.begin(), pattern.end());
    ...
    cout << vector<int>(
        stop, it); // интервал валдный, т.к. раз нашли, значит все норм
  }
  ...
  // поиск частей паттерна
  {
    ...
    while (true) {
      ...
      it = find_first_of(start, v.end(), // start <= end
  ...
  // equal - mismatch
  {
    ...
    pair<vector<int>::iterator, vector<int>::iterator> finded =
        mismatch(first.begin(), first.end(), v.begin());
    ...
    // Так можно сортировать массивы разноразмерный векторов. просто проверять
    // на эквив. нельзя. lexical - f < l
    assert(!lexicographical_compare(first.begin(), first.end(), second.begin(),
                                    second.end()));
    second.push_back(3);
    assert(lexicographical_compare(first.begin(), first.end(), second.begin(),
                                   second.end()));
