...
private:
    template <typename T>
    friend void registerBeanClass(T &obj);
    template <typename T>
    friend void drop(T &obj);
...
public:
    ...
    entities::Task persist(const entities::Task &v) { return self_->persist_(v); }

    void update(const entities::Task &v) { self_->update_(v); }

    entities::TaskEntities loadAll() const { return self_->loadAll_(); }
...
private:
    ...
    struct model : concept_t {
        ...
        virtual entities::Task persist_(const entities::Task &v) OVERRIDE { return data_.persist(v); }

        void update_(const entities::Task &v) OVERRIDE { data_.update(v); }

        entities::TaskEntities loadAll_() const OVERRIDE { return data_.loadAll(); }
...
template <>
struct holder_traits<std::string> {
...
/**
  ...
  \attention !!! How to know what thread delete it? Especialy if it's shared
  ptr! Hm... But not races if std::shared_ptr, destruction under lock?
*/
...
public:
    ...
    concepts::table_concept_t getTaskTableQuery() { return self_->getTaskTableQuery_(); }

    concepts::lifetime_concept_t getTaskLifetimeQuery() { return self_->getTaskLifetimeQuery_(); }

    concepts::table_concept_t getTagTableQuery() { return self_->getTagTableQuery_(); }
...
private:
    ...
    struct model : concept_t {
        ...
        concepts::table_concept_t getTaskTableQuery_() OVERRIDE { return data_.getTaskTableQuery(); }

        concepts::table_concept_t getTagTableQuery_() OVERRIDE { return data_.getTagTableQuery(); }

        concepts::lifetime_concept_t getTaskLifetimeQuery_() OVERRIDE { return data_.getTaskLifetimeQuery(); }
