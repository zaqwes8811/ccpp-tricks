...
// TODO: нельзя прочитать до истечения некоторого интервала времени после
вставки
...
class QueuePthread {
public:
    QueuePthread() { pthread_mutex_init(&_lock, NULL); }
    ~QueuePthread() { pthread_mutex_destroy(&_lock); }
    ...
    // нет это возвращение ссылки на то чего быть не должно - это из Саттера, но
    // не про то - T& pop();
    //
...
template <typename T>
class BlockedQueue : public boost::noncopyable  // DANGER: вообще то зависимотсь от буста
{
public:
    //
    ...
    void push(const T &src) {
        ...
        if (was_empty) pthread_cond_broadcast(&cond_);
    }
...
template <typename T>
class BlockedTimedQueue : public boost::noncopyable  // DANGER: вообще то зависимотсь от буста
{
public:
    //
    typedef T value_type;
...
template <typename T>
class BoundedBlockedQueue : public boost::noncopyable  // DANGER: вообще то зависимотсь от буста
{
public:
    //
    typedef T value_type;
    ...
    void push(const T &src) {
        ...
        ++size_;
        pthread_mutex_unlock(&lock_);
        if (was_empty) pthread_cond_broadcast(&rcond_);
    }
    ...
    void pop(T &dst) {
        ...
        pthread_mutex_unlock(&lock_);
        if (was_full) pthread_cond_broadcast(&wcond_);
    }
...
private:
    ...
    // не тужен вектор - его длина - O(1) - удаление из начала очень дорогая
    // операция
    // vector<T> array_;
