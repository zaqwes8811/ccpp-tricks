// Motivation:
//  Sean Parent - "No raw sync", Sutter, Intel TBB (планеровщик и task based
//  parallelism) Tasks better.
//
...
void thFun(int i, int &j) { std::cout << "Hi " << i << j << std::endl; }
...
void test(std::vector<std::thread> &workers) {
  ...
  for (int i = 0; i < 10; ++i) {
    ...
    // i !! by value
    auto action = [i]() { cout << "Hi from th: " << i << "\n"; };
    ...
    auto th = thread(
        thFun, i,
        ref(j)); // вообще проблема - висячие ссылки - not correct, but compiled
...
TEST(TBP, BanchOfThreads) {
  ...
  */
  ...
  // adobe::generate_n(workers, 10, builder);  // для созданных и в адобовской
  // либе _n нет
...
TEST(CC11, ConcurList) {
  ...
  // Scope
  {
    ...
    for (int i = 0; i < 10; ++i) {
      auto th = thread(f, ref(l)); // вообще проблема - висячие ссылки - not
                                   // correct, but compiled
      workers.push_back(move(th)); // th);
    }
    for_each(begin(workers), end(workers), mem_fun_ref(&thread::join));
    int t = l.count();
...
using namespace std;
void thFun(
    promise<string> &&prms) { // можно и по ссылке, тогда promise not invalidate
  // Exceptions
...
TEST(ThCpp11, Comm) {
  ...
  //
  future<string> ftr = prms.get_future(); // можно и в одном потоке использовать
                                          // - что-то связанное с исключениями
