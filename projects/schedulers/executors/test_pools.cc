...
TEST(ThPool, OwnAsioPool) {
    ...
    packaged_task<std::pair<int, int>> t(bind(bind(&Templ::doTheThing, ref(templ), _1), cref(in)));
...
TEST(ThPool, AsioBase) {
    ...
    for (int i = 0; i < 3; ++i) threads.create_thread(bind(&boost::asio::io_service::run, &io_service));
