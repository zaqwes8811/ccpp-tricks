...
// TROUBLE: для верий буста выше некоторой нужно TIME_UTC -> TIME_UTC_ - что-то
// связано с С11
...
int int_with_params(int param) { return param; }
...
void task_3() { print(string("  task_3()\n")); }
...
void run(int tNumber) {
  ...
  boost::exception_ptr error; // похоже одна на поток
  {
    std::auto_ptr<boost::asio::io_service::work> work(
        new boost::asio::io_service::work(svc)); // 3.
...
class Templ {
public:
  std::pair<int, int> doTheThing(const int &in) { return std::make_pair(0, 9); }
};
...
TEST(ThPool, OwnAsioPool) {
  ...
  packaged_task<std::pair<int, int>> t(
      bind(bind(&Templ::doTheThing, ref(templ), _1), cref(in)));
...
TEST(ThPool, AsioBase) {
  ...
  shared_ptr<task_t> example =
      boost::make_shared<task_t>(bind(&funcReturnStringWithParams, ans));
