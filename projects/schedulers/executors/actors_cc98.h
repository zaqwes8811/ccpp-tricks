...
public:
    ...
    AsioThreadPool() : m_io_service(), m_threads(), m_work(m_io_service) {
        m_threads.create_thread(boost::bind(&boost::asio::io_service::run, &m_io_service));
    }

    explicit AsioThreadPool(int countWorkers) : m_io_service(), m_threads(), m_work(m_io_service) {
...
public:
    void add(Func f) { m_io_service.post(f); }
...
public:
    ...
    Actor() : done(new Message), thd(new boost::thread(boost::bind(&Actor::Run, this))) {}
    ...
    // Enqueue a message
    void Send(boost::shared_ptr<Message> m) { mq.enqueue(m); }
};
}  // namespace cc98
...
public:
    ...
    static std::string getCurrentThreadId() { return boost::lexical_cast<std::string>(boost::this_thread::get_id()); }
