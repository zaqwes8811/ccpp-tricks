...
void startAccept(boost::asio::ip::tcp::acceptor &acceptor) {
  ...
  acceptor.async_accept(*socket, boost::bind(handleAccept,
...
public:
  ...
  async_udp_server(boost::asio::io_service &io_service)
      : m_socket(io_service), m_buffer(new vector<char>(1024, 0)) {
...
private:
  void start_receive() {
    m_socket.async_receive_from(
        buffer(&(*m_buffer)[0], m_buffer->size()), sender_endpoint,
  ...
  void handle_receive_from(const boost::system::error_code &error,
    ...
    if (!error && bytes_transferred > 0) {
      ...
      m_socket.async_send_to(
          ...
          bind(&async_udp_server::handle_send_to, this, message,
               boost::asio::placeholders::error,
               boost::asio::placeholders::bytes_transferred));
    } else {
      start_receive();
