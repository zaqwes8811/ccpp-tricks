...
void sqlite3::open(const std::string &filename) {
  if (sqlite3_open(filename.c_str(), &m_db_ptr)) {
    throw std::runtime_error(FROM_HERE + std::string("Can't open database: ") +
                             sqlite3_errmsg(*this));
  }
}
...
static int one_row_handler(void *targetPtr, int argc, char **argv,
                           char **azColName) {
  try {
    if (argc == 0)
      return 0;
...
Result sqlite3_exec(sqlite3 &db_ptr, const std::string &sql) {
  ...
  Result r;
  char *zErrMsg = 0;
  auto rc = ::sqlite3_exec(db_ptr.m_db_ptr, sql.c_str(),
                           &sqlite3_cc::one_row_handler, (void *)&r, &zErrMsg);
