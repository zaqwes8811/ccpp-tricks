...
shared_mutex::~shared_mutex() { std::lock_guard<mutex_t> _(mut_); }
...
upgrade_mutex::upgrade_mutex() : gate1_(), gate2_(), state_(0) {}

upgrade_mutex::~upgrade_mutex() { std::lock_guard<mutex_t> _(mut_); }
...
bool upgrade_mutex::try_lock_shared() {
  std::unique_lock<mutex_t> lk(mut_);
  count_t num_readers = state_ & n_readers_;
  if (!(state_ & write_entered_) && num_readers != n_readers_) {
    ++num_readers;
...
bool upgrade_mutex::try_lock_upgrade() {
  std::unique_lock<mutex_t> lk(mut_);
  count_t num_readers = state_ & n_readers_;
...
bool upgrade_mutex::try_unlock_shared_and_lock() {
  std::unique_lock<mutex_t> lk(mut_);
  if (state_ == 1) {
    state_ = write_entered_;
    return true;
...
void upgrade_mutex::unlock_and_lock_shared() {
  {
    std::lock_guard<mutex_t> _(mut_);
    state_ = 1;
  }
...
bool upgrade_mutex::try_unlock_shared_and_lock_upgrade() {
  std::unique_lock<mutex_t> lk(mut_);
  if (!(state_ & (write_entered_ | upgradable_entered_))) {
...
void upgrade_mutex::unlock_upgrade_and_lock_shared() {
  {
    std::lock_guard<mutex_t> _(mut_);
    state_ &= ~upgradable_entered_;
  }
...
void upgrade_mutex::unlock_upgrade_and_lock() {
  std::unique_lock<mutex_t> lk(mut_);
  count_t num_readers = (state_ & n_readers_) - 1;
  state_ &= ~(upgradable_entered_ | n_readers_);
...
bool upgrade_mutex::try_unlock_upgrade_and_lock() {
  std::unique_lock<mutex_t> lk(mut_);
...
void upgrade_mutex::unlock_and_lock_upgrade() {
  {
    std::lock_guard<mutex_t> _(mut_);
