...
template <class P> class DeleteArray {
public:
  inline static void Delete(const P *p) {
    /** @note If you see an error message about a negative subscript, that
     means your are attempting to use Loki to delete an incomplete type.
...
template <class P> struct CantResetWithStrong {
  inline bool OnReleaseAll(bool hasStrongPtr) const { return !hasStrongPtr; }

  inline bool OnResetAll(bool hasStrongPtr) const { return !hasStrongPtr; }
};
...
template <class P> struct AllowReset {
  inline bool OnReleaseAll(bool) const { return true; }
  inline bool OnResetAll(bool) const { return true; }
};
...
template <class P> struct NeverReset {
  inline bool OnReleaseAll(bool) const { return false; }
  inline bool OnResetAll(bool) const { return false; }
};
...
protected:
  inline explicit TwoRefLinks(bool strong) : m_pointer(0), m_strong(strong) {
    m_prev = m_next = this;
  }
...
public:
  StrongPtr(void) : OP(Strong) { KP::OnDefault(GetPointer()); }

  explicit StrongPtr(ExplicitArg p) : OP(p, Strong) {
    KP::OnInit(GetPointer());
  }

  StrongPtr(ImplicitArg p) : OP(p, Strong) { KP::OnInit(GetPointer()); }
  ...
  bool IsStrong(void) const { return Strong; }
