...
#define LOKI_THREADS_ATOMIC_FUNCTIONS                                          \
  ...
  static void AtomicAssign(IntType &lval, volatile const IntType &val) {       \
    InterlockedExchange(&lval, val);                                           \
                                                                               ...
                                                                               \
  static IntType AtomicIncrement(volatile IntType &lval,                       \
                                 const IntType compare, bool &matches) {       \
    ::EnterCriticalSection(&atomic_mutex_);                                    \
    ++lval;                                                                    \
...
class Mutex {
public:
  Mutex() LOKI_THREADS_MUTEX_CTOR(mtx_) { LOKI_THREADS_MUTEX_INIT(&mtx_); }
...
public:
  ...
  static IntType AtomicIncrement(volatile IntType &lval, const IntType compare,
                                 bool &matches) {
    ++lval;
    matches = (lval == compare);
    return lval;
  ...
  static IntType AtomicDecrement(volatile IntType &lval, const IntType compare,
                                 bool &matches) {
    --lval;
    matches = (lval == compare);
    return lval;
