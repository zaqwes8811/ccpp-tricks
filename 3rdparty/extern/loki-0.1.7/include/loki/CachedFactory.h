  ...
  void cleanVector() {
    ...
    if (currentTime - lastUpdate > timeValidity) {
      ...
      Vector::iterator newEnd =
          remove_if(m_vTimes.begin(), m_vTimes.end(),
                    bind2nd(less<clock_t>(), currentTime - timeValidity));
...
protected:
  ...
  void onDestroy(const DT &key) {
    using namespace std;
    m_vKeys.erase(
        remove_if(m_vKeys.begin(), m_vKeys.end(), bind2nd(equal_to<DT>(), key)),
        m_vKeys.end());
  }
    ...
    for (objVectorItr = fromKeyToObjVector.begin();
      ...
      objItr =
          remove_if(v.begin(), v.end(),
                    std::bind2nd(std::equal_to<AbstractProduct *>(), pProduct));
      if (objItr !=
