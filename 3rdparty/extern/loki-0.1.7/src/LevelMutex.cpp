...
LOKI_THREAD_LOCAL volatile ::Loki::LevelMutexInfo
    * ::Loki::LevelMutexInfo::s_currentMutex = nullptr;
...
GetFirstMutex(const ::Loki::LevelMutexInfo::MutexContainer &mutexes) {
  if (mutexes.size() == 0)
    return nullptr;
...
unsigned int GetLevel(const ::Loki::LevelMutexInfo::MutexContainer &mutexes) {
  ...
  return (nullptr == mutex) ? ::Loki::LevelMutexInfo::UnlockedLevel
                            : mutex->GetLevel();
}
...
LevelMutexInfo::MutexUndoer::MutexUndoer(MutexContainer &mutexes)
    : m_mutexes(mutexes), m_here(mutexes.end()) {
  assert(this != nullptr);
}
...
LevelMutexInfo::LevelMutexInfo(unsigned int level)
    : m_level(level), m_count(0), m_previous(nullptr) {
  assert(IsValid());
}
...
MutexErrors::Type
ThrowOnBadDesignMutexError::CheckError(MutexErrors::Type error,
                                       unsigned int level) {
...
SpinLevelMutex::SpinLevelMutex(unsigned int level) : m_mutex(), m_level(level) {
#if defined(_MSC_VER)
  ...
  case 0:
    return;
  case EBUSY:
    throw MutexException("pthread mutex already initialized!", level,
                         MutexErrors::AlreadyInitialized);
  default:
  case EINVAL:
    throw MutexException("pthread mutex has an invalid attribute!", level,
                         MutexErrors::InvalidAttribute);
  case EFAULT:
    throw MutexException("pthread mutex has an invalid address!", level,
                         MutexErrors::InvalidAddress);
  }
#endif
}
...
MutexErrors::Type SpinLevelMutex::Lock(void) volatile {
  ...
  default:
  case EINVAL:
    throw MutexException("pthread mutex not initialized properly!", GetLevel(),
                         MutexErrors::NotInitialized);
  case EFAULT:
    throw MutexException("pthread mutex is not valid!", GetLevel(),
                         MutexErrors::InvalidAddress);
  case EDEADLK:
...
MutexErrors::Type SpinLevelMutex::TryLock(void) volatile {
  ...
  case EBUSY:
    break;
  case EAGAIN:
    throw MutexException("pthread mutex reached recursion limit!", GetLevel(),
                         MutexErrors::TooMuchRecursion);
  }
...
SleepLevelMutex::SleepLevelMutex(unsigned int level)
    : SpinLevelMutex(level), m_sleepTime(1), m_wakable(true) {}
...
SleepLevelMutex::SleepLevelMutex(unsigned int level, unsigned int sleepTime)
    : SpinLevelMutex(level), m_sleepTime(sleepTime / 1000) {
  if (0 == m_sleepTime)
...
MutexException::MutexException(const char *message, unsigned int level,
                               MutexErrors::Type reason)
    : m_message(message), m_level(level), m_reason(reason) {}

// ----------------------------------------------------------------------------

MutexException::MutexException(const MutexException &that) throw()
    : ::std::exception(that), m_message(that.m_message), m_level(that.m_level),
...
const char *MutexException::what(void) const throw() { return m_message; }
...
MutexLocker::MutexLocker(volatile LevelMutexInfo &mutex,
...
MultiMutexLocker::MultiMutexLocker(LevelMutexInfo::MutexContainer &mutexes,
  ...
  m_locked = (MutexErrors::Success == result);
  if (!m_locked)
    throw MutexException("Unable to lock multiple mutexes.", GetLevel(mutexes),
                         result);
}
...
MultiMutexLocker::MultiMutexLocker(LevelMutexInfo::MutexContainer &mutexes,
  ...
  if (!lock)
    return;
  const MutexErrors::Type result =
      LevelMutexInfo::MultiLock(mutexes, milliSeconds);
  m_locked = (MutexErrors::Success == result);
  if (!m_locked)
    throw MutexException("Unable to lock multiple mutexes.", GetLevel(mutexes),
                         result);
