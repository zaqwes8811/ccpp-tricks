...
public:
  enum ThrowingPolicy { Never, Always, Randomly };
...
typedef ::Loki::LevelMutex<ExceptionTossingMutex, 1,
                           ::Loki::JustReturnMutexError, ::Loki::NoMutexWait>
    ExceptionMutex;
...
public:
  ...
  void Print(unsigned int value, unsigned int index,
  ...
  inline const volatile SleepMutex &GetMutex(void) const volatile {
    return m_mutex;
  }
...
template <class Thingy> class Unlocker {
public:
...
public:
  ...
  inline volatile ::Loki::LevelMutexInfo &GetMutex(void) volatile {
    return m_mutex;
  }

  inline const volatile ::Loki::LevelMutexInfo &GetMutex(void) const volatile {
    return m_mutex;
  }
