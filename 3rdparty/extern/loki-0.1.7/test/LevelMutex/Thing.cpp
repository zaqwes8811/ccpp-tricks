...
typedef ::Loki::LockingPtr<const Thing, volatile SleepMutex>
    ConstSingleThingLocker;

typedef ::Loki::LockingPtr<LevelThing, volatile SleepMutex> LevelThingLocker;

typedef ::Loki::LockingPtr<const LevelThing, volatile SleepMutex>
    ConstLevelThingLocker;

typedef ::Loki::LockingPtr<SomeThing, volatile SleepMutex> SomeThingLocker;

typedef ::Loki::LockingPtr<const SomeThing, volatile SleepMutex>
    ConstSomeThingLocker;
...
ExceptionTossingMutex::~ExceptionTossingMutex(void) { assert(NULL != this); }
...
MutexErrors::Type ExceptionTossingMutex::Lock(void) volatile {
  if (m_policy != ExceptionTossingMutex::Never) {
    ...
    if (toss) {
      throw MutexException("Just a test exception from DoLock.", GetLevel(),
                           ::Loki::MutexErrors::OtherError);
    }
  }
...
TestResults::TestResults(unsigned int threadCount) : m_results() {
  assert(NULL != this);
  Reset(threadCount);
}
...
TestResults::TestResult::TestResult(void) : m_total(0), m_fails(0) {
  assert(NULL != this);
}
...
TestResults::TestResult::TestResult(const TestResult &that)
    : m_total(that.m_total), m_fails(that.m_fails) {
  assert(NULL != this);
}

// ----------------------------------------------------------------------------

TestResults::TestResult::~TestResult(void) { assert(NULL != this); }
...
unsigned int Thing::GetPoolSize(void) { return s_pool.size(); }
...
Thing::Thing(unsigned int value) : m_mutex(10), m_value(value) {
  assert(NULL != this);
#if defined(_MSC_VER)
...
Thing::~Thing(void) { assert(NULL != this); }

// ----------------------------------------------------------------------------

void Thing::Print(unsigned int value, unsigned int index,
                  unsigned int startSize) const volatile {
  assert(NULL != this);
...
LevelThing::LevelThing(unsigned int level, unsigned int place)
    : m_mutex(level), m_place(place), m_value(0) {
  assert(NULL != this);
#if defined(_MSC_VER)
  m_mutex.GetMutexPolicy().SetSleepTime(5);
  m_mutex.GetMutexPolicy().SetWakable(false);
...
LevelThing::~LevelThing(void) { assert(NULL != this); }
...
SomeThing::SomeThing(unsigned int level, unsigned int place)
    : m_mutex(level), m_place(place), m_level(level), m_value(0) {
  assert(NULL != this);
#if defined(_MSC_VER)
  m_mutex.GetMutexPolicy().SetSleepTime(5);
  m_mutex.GetMutexPolicy().SetWakable(false);
...
SomeThing::~SomeThing(void) { assert(NULL != this); }
...
struct SomeThingComparator
    : std::binary_function<volatile SomeThing *, volatile SomeThing *, bool> {
  /// Returns true if left string < right string.
  inline bool operator()(volatile SomeThing *lhs,
                         volatile SomeThing *rhs) const {
