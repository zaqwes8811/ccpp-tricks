...
#define LokiThreadCreate(handle, attr, func, arg)                              \
  (int)((*handle = (HANDLE)_beginthreadex(nullptr, 0, func, arg, 0,            \
                                          nullptr)) == nullptr)
...
class Thread {
public:
  enum Status { Dead = 0, Idle, Starting, Active };

  inline static volatile Thread *GetCurrentThread(void) { return s_thread; }
  ...
  inline volatile ThreadPool *GetThreadPool(void) volatile { return m_owner; }
...
public:
  ...
  inline volatile Thread *GetThread(unsigned int index) volatile {
    return const_cast<volatile Thread *>(
        const_cast<const volatile ThreadPool *>(this)->GetThread(index));
  }
...
private:
  ...
  class Checker {
  public:
    inline explicit Checker(const volatile ThreadPool *mutex) : m_pool(mutex) {}
