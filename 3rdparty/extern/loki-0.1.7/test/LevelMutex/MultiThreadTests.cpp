...
void SetToRedoSingleTests(bool redo) { s_redo = redo; }

// ----------------------------------------------------------------------------

bool WillRedoSingleTests(void) { return s_redo; }
...
#define LOKI_pthread_create(handle, attr, func, arg)                           \
  (int)((*handle = (HANDLE)_beginthreadex(nullptr, 0, (ThreadFunction_)func,   \
                                          arg, 0, nullptr)) == nullptr)
...
void MultiThreadSimpleTest(void) {
  ...
  cout << endl
       << "Doing thread-locked print test.  This test should pass. and not "
          "deadlock"
       << endl;
  ::system("pause");
  ...
  cout << endl
       << "Doing thread-unsafe print test.  This test may fail." << endl;
  ::system("pause");
  for (unsigned int ii = 0; ii < threadCount; ++ii) {
    void *p = reinterpret_cast<void *>(ii);
...
void MultiThreadTryLockTest(void) {
  ...
  cout << endl
       << "Doing multi-threaded TryLock test.  This test should not deadlock."
       << endl;
  ::system("pause");
...
void MultiThreadReentrantTest(void) {
  ...
  cout << endl
       << "Doing thread-safe value test.  This test should pass and not "
          "deadlock."
       << endl;
  ::system("pause");
  for (unsigned int ii = 0; ii < threadCount; ++ii) {
  ...
  cout << endl
       << "Doing thread-unsafe value test.  This test may fail." << endl;
  ::system("pause");
  for (unsigned int ii = 0; ii < threadCount; ++ii) {
    void *p = reinterpret_cast<void *>(ii);
...
void MultiThreadMultiLockTest(void) {
  ...
  cout << endl
       << "Doing thread-safe multi-lock test.  This test should pass and not "
          "deadlock."
       << endl;
  ::system("pause");
  for (unsigned int ii = 0; ii < threadCount; ++ii) {
  ...
  cout << endl
       << "Doing thread-unsafe multi-lock test.  This test may fail." << endl;
...
void MultiThreadRandomMultiLockTest(void) {
  ...
  cout << endl
       << "Doing thread-safe random multi-lock test.  This test should pass "
          "and not deadlock."
       << endl;
  ::system("pause");
  for (unsigned int ii = 0; ii < threadCount; ++ii) {
  ...
  cout << endl
       << "Doing thread-unsafe random multi-lock test.  This test may fail."
       << endl;
  TestResults::GetIt()->Reset(threadCount);
  ::system("pause");
  for (unsigned int ii = 0; ii < threadCount; ++ii) {
...
void MultiThreadHierarchySingleLockTest(void) {
  ...
  cout << endl
       << "Doing thread-safe hierarchy test.  This test should pass and not "
          "deadlock."
       << endl;
  ::system("pause");
  for (unsigned int ii = 0; ii < threadCount; ++ii) {
  ...
  cout << endl
       << "Doing thread-unsafe hierarchy test.  This test may fail." << endl;
  ::system("pause");
  TestResults::GetIt()->Reset(threadCount);
  for (unsigned int ii = 0; ii < threadCount; ++ii) {
    void *p = reinterpret_cast<void *>(ii);
...
void *SafeHierarchyMultiLockTest(void *p) {
  ...
  try {
    for (unsigned int ii = 0; ii < 20; ++ii) {
      ...
      for (SomeThingPoolIter it(pool.begin()); it != pool.end(); ++it) {
...
void MultiThreadHierarchyMultiLockTest(void) {
  ...
  cout << endl
       << "Doing thread-safe multilock hierarchy test.  This test should pass "
          "and not deadlock."
       << endl;
  ::system("pause");
  for (unsigned int ii = 0; ii < threadCount; ++ii) {
  ...
  cout << endl
       << "Doing thread-unsafe multilock hierarchy test.  This test may fail."
       << endl;
  ::system("pause");
  TestResults::GetIt()->Reset(threadCount);
  for (unsigned int ii = 0; ii < threadCount; ++ii) {
