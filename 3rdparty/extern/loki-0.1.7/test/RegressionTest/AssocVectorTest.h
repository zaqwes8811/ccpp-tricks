...
public:
  ...
  template <class U> TestAllocator(const TestAllocator<U> &) {}
...
template <class Vect> bool is_sorted(const Vect &v) {
  if (v.size() < 2)
    return true;
  ...
  while (it != v.end()) {
    ...
    if (!v.key_comp()(previous, current))
      return false;
    previous = current;
    ++it;
  }
...
typedef Loki::AssocVector<int, int, std::less<int>,
                          TestAllocator<std::pair<int, int>>>
    test_vect1_t;
typedef Loki::AssocVector<const char *, int, str_less,
                          TestAllocator<std::pair<const char *, int>>>
    test_vect2_t;
typedef Loki::AssocVector<std::string, std::string> test_vect3_t;
typedef Loki::AssocVector<AVTestClass, AVTestClass, std::less<AVTestClass>,
                          TestAllocator<std::pair<AVTestClass, AVTestClass>>>
    test_vect4_t;
...
template <class Vect> void check_swap(Vect &v1, Vect &v2) {
...
void test_vect1() {
  ...
  // test_vect1_t vec99(test_vect1_t::key_compare(),
  // TestAllocator<test_vect1_t::value_type>()); assert(vec99.size() == 0); //-
  // here ir cries
  ...
  test_vect1_t *vec15 = new test_vect1_t(
      test_vect1_t::key_compare(), TestAllocator<test_vect1_t::value_type>());
  assert(vec15->size() == 0);
  check_insert1(*vec15);
  delete vec15;
...
void test_vect2() {
  ...
  std::pair<test_vect2_t::iterator, bool> aux =
      vec21.insert(std::make_pair("xyz", 99));
  assert(!aux.second);
...
void test_vect6() {
  ...
  for (unsigned int i = 0; i < numTests; ++i) {
    ...
    for (unsigned int i = 0; i < numSamples; ++i) {
      std::pair<int, int> newValue(rand() % numSamples, rand() % numSamples);
