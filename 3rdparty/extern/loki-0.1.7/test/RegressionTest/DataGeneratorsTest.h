...
struct DataGeneratorsTest : public Test {
  ...
  virtual void execute(TestResult &result) {
    ...
    typedef MakeTypelist<char, unsigned char, signed char, wchar_t>::Result
        char_types;
    ...
    // The following line of code stupidity is a direct result of the half-baked
    // idea
    ...
    sizes.reserve(n);
    typedef MakeTypelist<char, short, int, double>::Result some_types;
    iterate_types<some_types, sizeof_type>(std::back_inserter(sizes));
    size_t apriori_size[] = {sizeof(char), sizeof(short), sizeof(int),
                             sizeof(double)};
    b = true;
