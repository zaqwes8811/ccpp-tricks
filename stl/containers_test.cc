...
TEST(STL, Deque) {
  ...
  // Не совсем точно но:
  //   любая вставка/уд. делает недейств. итераторы => если программа хранит какие-то ссылки, то лучше его не выбирать
}
...
TEST(STL, List) {
  ...
  // добавляет в начало! может из-за того что двунаправленный список - нет не циркулярный список
  //l.insert(++l.end(), 90);  
  cout << l;
  l.remove(9);
  cout << l;
  ...
  //
  /// выделяется операция splice - она не копирует, что важно при работе с двуму разными списками
