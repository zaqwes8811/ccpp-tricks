    ...
    for (int size = 1024; size <= MAX_MSG_SIZE; size = size * 4) {
        ...
        if (rank == 0) {
            ...
            for (int i = 0; i < loop; i++) {
                MPI_Irecv(r_buf, size, MPI_CHAR, other_proc, 10, MPI_COMM_WORLD, &recv_request);
                MPI_Isend(s_buf, size, MPI_CHAR, other_proc, 100, MPI_COMM_WORLD, &send_request);
            ...
            for (int i = 0; i < loop; i++) {
                MPI_Irecv(r_buf, size, MPI_CHAR, other_proc, 100, MPI_COMM_WORLD, &recv_request);
