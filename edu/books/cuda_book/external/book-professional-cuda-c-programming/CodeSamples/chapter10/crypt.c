 ...
 */
static void encrypt_decrypt(encryptChunk *plainList, encryptChunk *cryptList, int *key, int plainLength) {
    if (plainLength % CHUNK_SIZE != 0) {
        fprintf(stderr,
                "Invalid encryption: length of plain must be an even "
...
void readInputData(FILE *in, size_t textLen, encryptChunk **textHeadOut, encryptChunk **textTailOut,
                   encryptChunk **cryptHeadOut, encryptChunk **cryptTailOut) {
    size_t nread = 0;
...
int main(int argc, char **argv) {
    ...
    if (textLen % CHUNK_SIZE != 0) {
        fprintf(stderr,
                "Invalid input file length %lu, must be evenly "
                "divisible by %d\n",
                textLen, CHUNK_SIZE);
        return (1);
    }
