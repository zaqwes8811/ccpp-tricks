...
void CUDART_CB mpi_callback(cudaStream_t stream, cudaError_t status, void *data) {
    ...
    MPI_Irecv(ctx->h_rcv, ctx->size, MPI_CHAR, ctx->other_proc, ctx->recv_id, MPI_COMM_WORLD, &(ctx->recv_request));
    MPI_Isend(ctx->h_src, ctx->size, MPI_CHAR, ctx->other_proc, ctx->send_id, MPI_COMM_WORLD, &(ctx->send_request));
    ...
    for (int size = 1024; size <= MAX_MSG_SIZE; size = size * 4) {
        ...
        if (rank == 0) {
            ...
            for (int i = 0; i < loop; i++) {
                 ...
                 */
                CHECK(cudaMemcpyAsync(h_src, d_src, size, cudaMemcpyDeviceToHost, stream));

                CHECK(cudaStreamAddCallback(stream, mpi_callback, &ctx, 0));
                /*
                 * Transfer the data received from the other MPI process to
                 * the device.
                 */
                CHECK(cudaMemcpyAsync(d_rcv, h_rcv, size, cudaMemcpyHostToDevice, stream));
            }
