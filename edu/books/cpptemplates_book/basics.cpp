...
// Update
template <typename T> struct AccumulationTraits; // traits template
...
template <typename T> auto accum_v2(T const *beg, T const *end) {
  using AccT = typename AccumulationTraits<T>::AccT;
...
template <typename T1, typename T2> class SumPolicyV1 {
public:
  static void accumulate(T1 &total, T2 const &value) { total += value; }
};

// T through all types
template <typename T, template <typename, typename> class Policy = SumPolicyV1,
          typename Traits = AccumulationTraits<T>>
auto accum_v4(T const *beg, T const *end) {
  typename Traits::AccT total = Traits::zero; // depend type?
  //    AccT total = AccumulationTraits<T>::zero();
...
// Acc with iterators
//
template <typename Iter> auto accum_v5(Iter start, Iter end) {
