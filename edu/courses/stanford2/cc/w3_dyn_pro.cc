...
struct ItemsCompare {
    bool operator()(const Item& lhs, const Item& rhs) { return !(lhs.w < rhs.w); }
};
...
int knapSack_hashtable(const TaskId& id, const vector<Item>& items, Store& store) {
    ...
    // Base Case
    if (n == 0 || w_bound == 0) return 0;

    // If weight of the nth item is more than Knapsack capacity W, then
    // this item cannot be included in the optimal solution
    if (items[n - 1].w > w_bound) {
        return knapSack_hashtable(TaskId(w_bound, n - 1), items, store);
    } else {
        // Return the maximum of two cases: (1) nth item included (2) not included
        int sum_values =
            std::max(knapSack_hashtable(TaskId(w_bound - items[n - 1].w, n - 1), items, store) + items[n - 1].v,
...
// template <typename Store>
//  Too slooow
int knapSack_hashtable_2d(const TaskId& id, const vector<Item>& items,
    ...
    // check task is solved
    {
        ...
        if (store.end() != it) {
            ...
            if (tmp.end() != tmp.find(id.idx)) return tmp[id.idx];
        }
    }
    ...
    // this item cannot be included in the optimal solution
    if (items[n - 1].w > w_bound) {
        return knapSack_hashtable_2d(TaskId(w_bound, n - 1), items, store);
    } else {
        // Return the maximum of two cases: (1) nth item included (2) not included
        int sum_values =
            std::max(knapSack_hashtable_2d(TaskId(w_bound - items[n - 1].w, n - 1), items, store) + items[n - 1].v,
