...
TEST(W3, ParallelKnap) {
    ...
    TaskId root(W, count);
    // int store = 0;
    KnapsackTaskSerial<TaskTable>& a =
        *new (task::allocate_root()) KnapsackTaskSerial<TaskTable>(items, g_task_table, result, root);
...
TEST(W3, ParallelKnapNoWait) {
    ...
    TaskId root(W, count);
    // int store = 0;
    KnapsackTaskFuture<TaskTable>& a =
        *new (task::allocate_root()) KnapsackTaskFuture<TaskTable>(items, g_task_table, result, root);
    task::spawn_root_and_wait(a);
    // KnapsackTaskSerial<int>(items, store, result, root).execute();
