...
int main() {
    ...
    try {
        ...
        for (size_t i = 0; i < track.size(); ++i) info[i].idx = i;
        int v_active = 1;
        track[v_active].d = 0;
        ...
        while (true) {  // как выйти то? А если граф не связан!?
            ...
            vector<NodeInfo>::iterator at =
                min_element(begin(track), end(track), [](const NodeInfo& o1, const NodeInfo& o2) -> bool {
                    int d1 = o1.d;
                    int d2 = o2.d;
            ...
            for_each_(Neighbor n, neighbors) {
                ...
                if (!track[v].visited) {
                    ...
                    int new_d = root_d + w;
                    if (current_d > new_d) track[v].d = new_d;
                }
            }
        }
        ...
        for_each(begin(mask_s), end(mask_s), [&track](int i) { cout << track[i].d << ","; });
        cout << endl;
