...
template <typename Iterator> void my_advance(Iterator &iter, int n) {
  //    if (...) iter+=n;
  //    else ...
...
template <bool B, typename U, typename V> struct condtional { using type = V; };

template <typename U, typename V> struct condtional<true, U, V> {
  using type = V;
};
...
private:
  ...
  template <bool IsConst> struct common_iterator {
  private:
    //        using CondtionalPtr = ...
...
public:
  ...
  void reserve(size_t n) {
    ...
    T *newarr =
        reinterpret_cast<T *>(new int8_t[n * sizeof(T)]); // MY: alignment
...
class C {
  template <typename U> C(const U &) = delete;
};
