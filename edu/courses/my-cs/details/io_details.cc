...
pair<int, vector<Item>> get_dyn_items(const string& filename) {
    ...
    if (!stream) throw runtime_error("Error: can't open file");
    ...
    while (!stream.fail()) {
        ...
        ss >> i;
        if (!ss) break;
        if (ss.peek() == kSplitter) ss.ignore();

        // W
        int j = 0;
        ss >> j;
        if (!ss) throw invalid_argument("Error: String format is broken.");
        if (ss.peek() == kSplitter) ss.ignore();
    ...
    if (items.size() != count_items) throw runtime_error("Error: brocken file  " + filename);
...
pair<int, vector<Arrow>> parse_node_data(const string& line, stringstream& ss) {
    ...
    while (true) {
        ...
        // Find max
        if (i > max_node_idx) max_node_idx = i;

        // W
        int j = 0;
        bool w = ss >> j;
        if (!w) throw invalid_argument("Error: String format is broken.");
        if (ss.peek() == kSplitter) ss.ignore();
