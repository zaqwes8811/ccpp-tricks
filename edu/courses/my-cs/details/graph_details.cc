...
public:
    ...
    pair<int, Neighbors> operator()(const string& arg) const {
        ...
        Neighbors n;
        for_each_(Arrow a, raw_code.second) { n.push_back(EdgeMaker(a)); }
...
vector<string> extract_records(const string& filename) {
    ...
    if (!stream) throw std::runtime_error("Error: can't open file");
...
vector<pair<int, Neighbors>> process_parallel(const vector<string>& records) {
    ...
    // Trouble: "видимо у вас проблема в том, что вы ставите 486-е ядро, а оно видит только одно ядро процессора.
    // ставьте 686-е"
    //   uname -r  # посмотреть что за ядро
    //
    { parallel_for(blocked_range<int>(0, records.size()), ApplyFoo(&records[0], &raw_nodes[0]), simple_partitioner()); }
#endif
    return raw_nodes;
}
...
vector<pair<int, Neighbors>> process_serial(const vector<string>& records) {
    ...
    if (true) {
        ...
        { std::transform(records.begin(), records.end(), tmp.begin(), RawYieldFunctor()); }
    }
    return tmp;
}
...
vector<Neighbors> build_graph(const vector<string>& records) {
    ...
    auto action = [&unique_check](const pair<int, Neighbors>& val) { unique_check.insert(val.first); };
    ...
    for_each_(const value_type elem, raw_nodes) { graph[elem.first] = elem.second; }
    return graph;
}
}  // namespace graph_details
