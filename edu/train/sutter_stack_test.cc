// Нейтральность нужна. Т.к. обобщенный контейнер не знает ничего о типах хр. в нем элементов, а вызывающий код должен
// знать.
...
Stack<T>::Stack(const Stack<T>& other)
    : v_(NewCopy(other.v_, other.vsize_,
...
public:
    TestThrow() {
        i++;
        if (i == 5) throw std::runtime_error("");
    }
    ~TestThrow() { cout << "dtr\n"; }
};
...
public:
    explicit StackInh(size_t size = 0) : StackImpl<T>(size) {}
    ...
    void Push(const T& t) {
        if (this->vused_ == this->vsize_) {
            ...
            while (temp.Count() < this->vused_) temp.Push(this->v_[temp.Count()]);
            temp.Push(t);
            this->Swap(temp);
        } else {
            using util::construct;
...
// если какая-то часть объекта при конструировании сгенерировала исключение, то и весь констр. должен сгенерировать
// искл.
...
TEST(Exc, Composite) { EXPECT_ANY_THROW(compose()); }
