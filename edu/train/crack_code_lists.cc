...
public:
    ...
    void push_back(const T& elem) {
        ...
        node* const ptr = new node(elem, 0);
        // tmp.release();
        if (head_) {
            tail_->next = ptr;
    ...
    node* get_nth_to_last(const int nth) const {
        ...
        for (int i = 0; i < nth - 1; ++i) {
            if (!p2->next) return 0;  // нет столько элементов
...
public:
    ...
    own_forward_list<T>& operator=(own_forward_list<T> rhs  // по значению
        ...
        // tmp.head_ = 0;  // BUG!! нужно либо swap и данный левого удалить временный объект, либо как-то обменять
        // указатели tmp.tail_ = 0;
        //}
        return *this;
    }
    ...
    // FIXME: сделать exception-safe - проблема с том, что если возникнет искл. при конструировании, то деструктор не
    // вызовется
    //   и память утечет. Может catch(...) throw; Похоже не всегда катит. Если члены - values - то вообще проблема, см.
    //   у Саттера
...
private:
    ...
    //   проблемы возникнут, т.к. некоторое время оба указателя ссылаются на один и тот же объект, хотя, по сути нам
    //   нужен только head Кажется ради удаления пары строчек, не стоит изголятся.
    node* head_;
