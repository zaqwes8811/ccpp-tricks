...
struct node {
    ...
    explicit node(int _data)
        : left(0),
          right(0)  // node() - c++11 delegation ctor
          ,
          data(_data) {}
    node* left;
    node* right;
...
void post_order_tree(node* root, int level) {
    ...
    // extra space
    if (!root->left && !root->right) cout << root->data << "/" << level << " ";
}
...
void print_tree_pre_order_iterative(node* const root) {
    ...
    while (top) {
        ...
        // правый уходит вниз
        if (top->right) parent_stack.push(top->right);

        if (top->left) parent_stack.push(top->left);
