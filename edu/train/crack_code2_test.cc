...
bool there_unique(const string& line) {
    ...
    //
    // V1:
    //   в хэштаблице быстро искать - O(1) - в качестве замены можно использовать просто массив по числу байт, а
    //   вычислять хэш
    //
    // V2:
...
void unique_n2(char* str, const unsigned size) {
    ...
    for (int current = 1; current < size; ++current) {
        ...
        while (finded < end) {
            if (str[finded] == elem) break;  // нашли. просто переходим к следующему элементу
            ++finded;
        }
...
bool isSubstring(const string& where, const string& what) { return false; }
...
TEST(Crack, ArrAndStrings) {
    ...
    const int N = 5;
    int mat_raw[N][N] = {
        {11, 12, 13, 14, 92}, {15, 16, 17, 18, 92}, {19, 10, 11, 12, 92}, {13, 14, 15, 16, 92}, {70, 70, 70, 70, 70}};
