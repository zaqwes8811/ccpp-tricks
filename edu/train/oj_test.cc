...
struct spaces_purger {
    bool operator()(string::value_type f, string::value_type n) const { return isspace(f) && isspace(n); }
};
...
TEST(OJ, Reverse) {
    ...
    s.erase(unique(s.begin(), s.end(), spaces_purger()),
            s.end());  // O(n)
    ...
    for (int i = 0; i < 0; ++i) {  // exp time stop...
        ...
        for (vector<string>::iterator elem = store.begin();;) {
            ...
            // calc
            {
                ...
                string tmp;
                while (true) {
                    if (tail.begin() + offset == tail.end()) break;
            ...
            if (dict.find(tail) == dict.end()) {
                elem = store.erase(elem);  // строка тупиковая и похоже отсюда и идет экспоненциальность - в ней много
                                           // одинаковых подзадач
            } else {
                ++elem;
            }
...
TEST(O_J, WordBreak2List) {
    ...
    for (int i = 0; i < 100000; ++i) {
        ...
        for (
            // vector
            list<string>::iterator elem = store.begin();;) {
            // много на себя берет - и вставляет и удаляет
