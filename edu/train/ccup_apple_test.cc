...
template <typename T  // must non throw copy functions;
          >
...
public:
    ...
    write_and_read(T* s  // ring buffer ptr
                   ,
                   int n)
        : m_start(s), m_producer_ptr(s), m_consumer_ptr(s), m_max_count_cells(n), m_num_of_cells(0) {}
    ...
    bool write(T* in_ptr, int n) {
        if (in_ptr == NULL && n > 0) return false;

        size_t requested_cells = n * sizeof(T) / sizeof(char);

        if (m_max_count_cells - m_num_of_cells < requested_cells) return false;
    ...
    T* read(int n) {
        ...
        if (count < n) m_consumer_ptr = m_start;
