...
public:
    StreamGenerator() : ptr(0), stream("The angry dog was red. And the cat was also angry.") {}
...
template <typename T>
struct OnQueue {
    OnQueue() : m_buffer(3), m_rd_ptr(0), m_wr_ptr(0), m_can_wr(true), m_can_rd(false) {}

    // must be O(1)
    bool push(const T& val) {
        if (!m_can_wr) return false;
    ...
    // bad api
    bool pop(T& r_val) {
        if (!m_can_rd) return false;

        r_val = m_buffer[m_rd_ptr];
        m_rd_ptr = inc(m_rd_ptr);

        m_can_wr = true;
        if (m_rd_ptr == m_wr_ptr) m_can_rd = false;
...
TEST(Amazon, OptBigBin_mod) {
    ...
    const size_t countPoints = 1024 * 2 * 2;
    ...
    if (file.is_open()) {
        while (!file.eof()) {
            ...
            for (size_t j = 0; j < file.gcount(); ++j) sum += buffer[j];
        }
    }
    file.close();
}
