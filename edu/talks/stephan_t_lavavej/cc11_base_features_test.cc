...
// Еще что-то про uniaue_ptr
// http://www.informit.com/guides/content.aspx?g=cplusplus&seqNum=495
...
TEST(Cpp11, SimpleLambda) {
    ...
    int total = 0;
    std::for_each(someList.begin(), someList.end(), [&](int x) { total += x; });
}
...
template <class T>
std::auto_ptr<T> auto_new() {
    return std::auto_ptr<T>(new T());
}

template <class T>
std::auto_ptr<T> auto_new(int a1) {
    return std::auto_ptr<T>(new T(a1));
}

template <class T>
std::auto_ptr<T> auto_new(int a1, int a2) {
...
template <class T>
void func(T &&t) {}
// Don't let T&& fool you here - t is not an rvalue reference [2]. When it
// appears in a type-deducing context, T&& acquires a special meaning.
