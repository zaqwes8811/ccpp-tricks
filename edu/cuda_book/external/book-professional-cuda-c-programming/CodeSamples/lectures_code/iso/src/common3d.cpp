 ...
 */
void save_layer_text(TYPE *field, int z, const int dimx, const int dimy, const int ny, const int nx,
    ...
    for (int y = 0; y < ny; y++) {
        for (int x = 0; x < nx; x++) {
            fprintf(fp, "%d %d %.20f\n", y, x, field[POINT_OFFSET(x, y, z, dimy, dimx, radius)]);
        }
        fprintf(fp, "\n");
    }
...
void init_data(TYPE *curr, TYPE *next, TYPE *vsq, TYPE *h_coeff, const int dimx, const int dimy, const int dimz,
...
void usage(char **argv) {
    fprintf(stderr,
            "usage: %s [-v] [-s] [-x nx] [-y ny] [-z nz] [-i iters] "
