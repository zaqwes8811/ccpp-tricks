...
void save_layer_text(TYPE *field, int z, const int dimx, const int dimy,
                     const int ny, const int nx, const char *filename,
                     int radius) {
...
void init_data(TYPE *curr, TYPE *next, TYPE *vsq, TYPE *h_coeff, const int dimx,
               const int dimy, const int dimz, const TYPE dx, const TYPE dt) {
  // init the vsq array -------------------------
...
void setup_config(config *conf, int argc, char **argv) {
  ...
  while ((c = getopt(argc, argv, "x:y:z:i:svr:t:p:g:w:d")) != -1) {
    ...
    case 'p':
      conf->srcs =
          (source *)realloc(conf->srcs, sizeof(source) * (conf->nsrcs + 1));
